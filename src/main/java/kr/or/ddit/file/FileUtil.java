...
public class FileUtil {
	 ...
	 * ex ) form-data; name="uploadFile"; filename="cony.png"
	 * return : sally.png
	 */
	public static String getFileName(String contentDispostion){
		...
		for(String fileStr : fileNames){
			if(fileStr.startsWith("filename")){ // filename으로 시작하면 true
				...
				String[] fileValue = fileStr.split("="); //filename="cony.png" 를 '=' 단위로 잘라서 저장
				file = fileValue[1].replaceAll("\"", "");// [0] = filename , [1] = "cony.png" 
				
				// fileName = fileStr.substring("filename".length()+2, fileStr.length()-1);
				// break;
			}
		}
